            ...
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                {round?.questions.map((q, idx) => (
                  <div key={q.id} className="bg-white border-2 border-slate-200 rounded-2xl p-4 space-y-2">
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="text-[10px] font-mono text-slate-400 uppercase">Q{idx + 1} · {q.type}</span>
                        <p className="text-xs font-bold text-slate-800 mt-0.5">{q.title}</p>
                      </div>
                      {submission.answers[q.id] ? <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" /> : <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />}
                    </div>
                    <div className={`p-3 rounded-xl text-xs font-mono ${q.type === 'code' ? 'bg-slate-900 text-emerald-300' : 'bg-slate-50 text-slate-700 border border-slate-200'}`}>
                      {submission.answers[q.id] || <span className="text-slate-400 italic">No answer provided</span>}
                    </div>
                  </div>
                ))}
                {!round && Object.entries(submission.answers).map(([qId, ans]) => (
                  <div key={qId} className="bg-white border-2 border-slate-200 rounded-2xl p-4">
                    <p className="text-[10px] font-mono text-slate-400 uppercase">{qId}</p>
                    <p className="text-xs text-slate-700 mt-1">{ans || '(No answer)'}</p>
                  </div>
                ))}
              </div>
